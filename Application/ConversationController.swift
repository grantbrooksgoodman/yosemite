...
class ConversationController: UIViewController, MFMailComposeViewControllerDelegate {
    ...
    @IBOutlet weak var tableViewMessagesConstraint: NSLayoutConstraint!
    //==================================================//
    ...
    var messagesConstraint: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        ...
        guard let conversations = conversationArray else { //No conversations.
            report("No Conversations to load.", errorCode: nil, isFatal: false, metadata: [#file, #function, #line])
            return
        }
        
        //Set the «otherUser» variable on each of the Conversations.
        ConversationSerializer().setOtherUsers(for: conversations) { (setOtherUsersError) in
            ...
            } else { //Successfully set the other Users.
        ...
        for individualConversation in conversations {
            /*
             Monitor changes to the «lastModified» variable of each serialized Conversation.
             If it can be converted into a date, update its correspondoing deserialized instance's «lastModifiedDate» variable.
             */
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        topConstraint = tableViewTopConstraint
        messagesConstraint = tableViewMessagesConstraint
        ...
        DispatchQueue.main.async {
            if let conversationArray = self.conversationArray {
                ...
                if notSetConversations.count > 0 /* If there are Conversations that don't have their «otherUser» set yet. */ {
                    openStream(forFile: #file, forFunction: #function, forLine: #line, withMessage: "Some conversations need to be set up...")
                    
                    logToStream(forLine: #line, withMessage: "Setting up conversations.")
                    
                    //Set the «otherUser» variable on each of the Conversations.
                    ConversationSerializer().setOtherUsers(for: self.conversationArray!.filter({$0.otherUser == nil})) { (setOtherUsersError) in
                        if let setOtherUsersError = setOtherUsersError {
                            closeStream(onLine: #line, withMessage: "Failure!")
                            ...
                            closeStream(onLine: #line, withMessage: "Success!")
                        }
                    }
    ...
    func setUpMatchView() {
        ...
        if let matches = matchIdentifiers,
           matches.count > 0 /* If we have matches to process. */ {
            ...
            UserSerializer().getUsers(withIdentifiers: matches) { (wrappedUsers, wrappedErrorDescriptors) in
                if let returnedUsers = wrappedUsers /* If we got the Users successfully. */ {
                    ...
                    self.tableViewTopConstraint = self.topConstraint
                    self.tableViewMessagesConstraint = self.messagesConstraint
                    
                    self.tableViewTopConstraint.isActive = false
                    self.tableViewMessagesConstraint.isActive = true
                    ...
                    guard let errorDescriptors = wrappedErrorDescriptors else { //No error descriptors.
            ...
            tableViewTopConstraint = topConstraint
            tableViewMessagesConstraint = messagesConstraint
            
            tableViewMessagesConstraint.isActive = false
    ...
    func setVisualElements(of cell: ChatCell, with data: Conversation) -> ChatCell? {
        ...
        guard let otherUser = data.otherUser else {
            return nil
        }
        ...
        cell.previewTextView.text = data.associatedMessages.last!.messageContent!
        //cell.previewTextView.contentOffset = .zero
        //cell.previewTextView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
        ...
        } else { //If the User does not have an avatar image, or it couldn't be converted to data.
            //Set the «avatarView» to a generated Avatar using the User's first name.
            cell.avatarView.set(avatar: Avatar(image: nil, initials: "\(otherUser.firstName.characters[0].uppercased())"))
        }
        ...
        for individualCell in visibleCells {
            ...
            if let chatCell = individualCell as? ChatCell,
               let conversationForCell = conversationArray!.first(where: { aTagFor($0.associatedIdentifier) == chatCell.tag }) {
                ...
                chatCell.dateLabel.text = conversationForCell.lastModifiedDate.elapsedInterval()
                
                if chatCell.dateLabel.alpha == 0 {
                    UIView.animate(withDuration: 0.2) {
                        chatCell.dateLabel.alpha = 1
                    }
                }
            }
        }
...
extension ConversationController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        guard let otherUser = self.conversationArray![indexPath.row].otherUser else {
            report("No «otherUser».", errorCode: nil, isFatal: false, metadata: [#file, #function, #line])
            return nil
        }
            ...
            AlertKit().confirmationAlertController(title: "Are you sure?",
                ...
                if let didConfirm = wrappedDidConfirm,
                   didConfirm /*If the user confirmed that they would like to unmatch with this User. */ {
                    ...
                    ConversationSerializer().deleteConversation(selectedConversation) { (wrappedErrorDescriptor) in
                        if let deleteConversationError = wrappedErrorDescriptor /* If the Conversation couldn't be deleted. */ {
                            ...
                            UserSerializer().removeMatch(between: currentUser!.associatedIdentifier, and: otherUser.associatedIdentifier) { (removeMatchError) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        Database.database().reference().child("allConversations").child(conversationArray![indexPath.row].associatedIdentifier).child("associatedMessages").observe(.childAdded, with: { (returnedSnapshot) in
            if let newMessageIdentifier = returnedSnapshot.value as? String /* If the Message's identifier could be unwrapped. */ {
                ...
                if self.conversationArray!.count <= indexPath.row {
                    ...
                    if shouldAddMessage {
                        //Get and deserialize the Message.
                        MessageSerializer().getMessage(withIdentifier: newMessageIdentifier) { (wrappedMessage, getMessageError) in
        ...
        } else { //No «ChatCell» was returned, i.e. there was no «otherUser».
            //Report the error.
...
extension ConversationController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let currentCell = collectionView.cellForItem(at: indexPath) as? MatchCell {
            ...
            ConversationSerializer().createConversation(initialMessageIdentifier: "!", participantIdentifiers: [currentUser!.associatedIdentifier, matchArray![indexPath.row].associatedIdentifier]) { (wrappedIdentifier, createConversationError) in
