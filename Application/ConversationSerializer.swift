...
class ConversationSerializer {
    ...
    func createConversation(initialMessageIdentifier: String, participantIdentifiers: [String], completionHandler: @escaping(_ returnedIdentifier: String?, _ errorDescriptor: String?) -> Void) {
        ...
        if let generatedKey = Database.database().reference().child("/allConversations/").childByAutoId().key {
            ...
            GenericSerializer().updateValue(onKey: "/allConversations/\(generatedKey)", withData: dataBundle) { (wrappedError) in
                if let returnedError = wrappedError {
                    ...
                    UserSerializer().getUsers(withIdentifiers: participantIdentifiers) { (wrappedUsers, wrappedErrorDescriptors) in
                        if let returnedUsers = wrappedUsers {
                            ...
                            for individualUser in returnedUsers {
                                ...
                                GenericSerializer().setValue(onKey: "/allUsers/\(individualUser.associatedIdentifier!)/openConversations", withData: mutableConversations == nil ? [generatedKey] : mutableConversations!) { (wrappedError) in
                            ...
                            guard let getUsersErrors = wrappedErrorDescriptors else {
                                if verboseFunctionExposure { print("Finished creating Conversation!") }
                                completionHandler(nil, "An unknown error occurred.")
                                return
                            }
        ...
        Database.database().reference().child("allConversations").child(withIdentifier).observeSingleEvent(of: .value, with: { (returnedSnapshot) in
            if let returnedSnapshotAsDictionary = returnedSnapshot.value as? NSDictionary,
               let asDataBundle = returnedSnapshotAsDictionary as? [String: Any] {
                ...
                self.deSerializeConversation(fromDataBundle: mutableDataBundle) { (wrappedConversation, deSerializeErrorDescriptor) in
                    if let returnedConversation = wrappedConversation {
                        ...
                        completionHandler(nil, deSerializeErrorDescriptor!)
                    }
                }
            } else {
                if verboseFunctionExposure { print("Finished getting Conversation!") }
            ...
            if let removeConversationReferenceErrors = wrappedRemoveConversationReferenceErrorDescriptors {
                ...
                if conversation.associatedMessages.count == 0 {
                    ...
                    if let messageIdentifiers = conversation.messageIdentifiers() {
                        MessageSerializer().deleteMessages(withIdentifiers: messageIdentifiers) { (wrappedDeleteMessagesErrorDescriptors) in
    ...
    func removeConversation(_ withIdentifier: String, completionHandler: @escaping(_ errorDescriptor: String?) -> Void) {
        GenericSerializer().updateValue(onKey: "/allConversations", withData: [withIdentifier: NSNull()]) { (wrappedError) in
        ...
        if fromUsers.count != 0 {
            for individualUser in fromUsers {
                if let openConversations = individualUser.openConversations {
                    ...
                    GenericSerializer().setValue(onKey: "/allUsers/\(individualUser.associatedIdentifier!)/openConversations", withData: (filteredConversations.count == 0 ? ["!"] : filteredConversations)) { (wrappedError) in
                        if let setValueError = wrappedError {
                ...
                } else { //This must be a new match that wasn't acted upon.
    ...
    func setOtherUser(for conversation: Conversation, completion: @escaping(_ errorDescriptor: String?) -> Void) {
        ...
        UserSerializer().getUser(withIdentifier: otherUserIdentifier) { (wrappedUser, getUserError) in
    ...
    func setOtherUsers(for conversations: [Conversation], completion: @escaping(_ errorDescriptor: String?) -> Void) {
        ...
        for individualConversation in conversations {
            ConversationSerializer().setOtherUser(for: individualConversation) { (error) in
    ...
    private func deSerializeConversation(fromDataBundle: [String: Any], completionHandler: @escaping(_ deSerializedConversation: Conversation?, _ errorDescriptor: String?) -> Void) {
        if verboseFunctionExposure { print("Deserializing Conversation...") }
        ...
        guard let associatedMessages = fromDataBundle["associatedMessages"] as? [String] else {
            completionHandler(nil, "Unable to deserialize «associatedMessages».")
            return
        }
        
        guard let conversationParticipants = fromDataBundle["conversationParticipants"] as? [String] else {
            completionHandler(nil, "Unable to deserialize «conversationParticipants».")
            return
        }
        
        guard let lastModified = fromDataBundle["lastModified"] as? String else {
            completionHandler(nil, "Unable to deserialize «lastModified».")
            return
        }
        
        if let fullyDeSerializedLastModifiedDate = secondaryDateFormatter.date(from: lastModified) {
            MessageSerializer().getMessages(withIdentifiers: associatedMessages) { (wrappedMessages, getMessagesStatus) in
                if let returnedMessages = wrappedMessages {
                    if verboseFunctionExposure { print("Finished deserializing Conversation!") }
                    
                    let deSerializedConversation = Conversation(associatedIdentifier: associatedIdentifier, associatedMessages: returnedMessages, lastModifiedDate: fullyDeSerializedLastModifiedDate, participantIdentifiers: conversationParticipants)
                    
                    completionHandler(deSerializedConversation, nil)
